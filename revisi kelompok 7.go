...
func cariInvestasi() {
	...
	switch metode {
	case 1:
		hasil = cariDenganSequential(target)
	case 2:
		hasil = cariDenganBinary(target)
	default:
...
func cariDenganSequential(nama string) *Investasi {
	for i := 0; i < len(investasiList); i++ {
		if investasiList[i].Nama == nama {
			return &investasiList[i]
		}
	}
	return nil
}

func cariDenganBinary(nama string) *Investasi {
	// Urutkan berdasarkan nama sebelum pencarian binary
	sort.Slice(investasiList, func(i, j int) bool {
		return investasiList[i].Nama < investasiList[j].Nama
	})

	low := 0
	high := len(investasiList) - 1

	for low <= high {
		mid := (low + high) / 2
		if investasiList[mid].Nama == nama {
			return &investasiList[mid]
		} else if investasiList[mid].Nama < nama {
			low = mid + 1
		} else {
			high = mid - 1
		}
